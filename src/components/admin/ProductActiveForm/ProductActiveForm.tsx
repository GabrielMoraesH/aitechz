...
export function ProductActiveForm({ active, action, className }: { active: boolean; action: () => Promise<void>; className?: string }) {
...
function ToggleButton({ active, className, triggerRef, open, onOpen, onClose, onConfirm }: { active: boolean; className?: string; triggerRef: RefObject<HTMLButtonElement | null>; open: boolean; onOpen: () => void; onClose: () => void; onConfirm: () => void }) {
  ...
  return <><button ref={triggerRef} type={active ? "button" : "submit"} className={className} disabled={pending} aria-label={`${active ? "Desativar" : "Reativar"} produto`} onClick={active ? onOpen : undefined}>{pending ? (active ? "Desativando..." : "Reativando...") : active ? "Desativar" : "Reativar"}</button><ConfirmDialog open={open} title="Desativar produto?" description="O produto deixará de aparecer no site, mas poderá ser reativado depois." confirmLabel="Desativar" pendingLabel="Desativando..." variant="danger" pending={pending} triggerRef={triggerRef} onCancel={onClose} onConfirm={onConfirm} /></>;
}
